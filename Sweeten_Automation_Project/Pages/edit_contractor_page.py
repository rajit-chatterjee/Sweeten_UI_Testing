'''
Functionalities of Edit Contractor Page
'''
...
log=logger.set_log("edit contractor page")
...
def edit_design_service(browser,status):
    ...
    if status=="yes":
        ...
        cf.checkbox_deselect(browser, locator.contractor_designservice_id, design_service_status, locator.find_by_id)
...
def edit_build_service(browser,status):
    ...
    if status=="yes":
        ...
        cf.checkbox_deselect(browser, locator.contractor_buildservice_id, build_service_status, locator.find_by_id)
        log.info("build service is unchecked")

#Submit project
def click_submit(browser):
    submit_btn=cf.find_an_element(browser, locator.find_by_xpath, locator.editcontractor_submitcontractor_xpath)
    cf.click_element(submit_btn)
    log.info("edited Contractor is submitted")